...
def test_fetch_json_success(monkeypatch):
    ...
    monkeypatch.setattr(api.requests, "get", make_fake_get(expected_data, status_ok=True))
...
def test_get_participants(monkeypatch):
    captured = {}
    monkeypatch.setattr(api.requests, "get", make_fake_get({"items": [101, 202]}, captured=captured))
...
def test_require_values_all_present_ok():
    require_values(context="All present", participant=1, datestr="2024-01-01", direction="in", event=1)
