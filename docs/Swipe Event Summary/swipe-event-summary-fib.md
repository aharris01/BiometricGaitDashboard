...
| Get footsteps data | `GET /api/events/{event_id}/footsteps/data`| `event_id` | `[{"footstep_id": id, "p100": [2d array data], "grf": [1d array data]}]` | Footstep data should be stored on the frontend somewhere using dcc.Store |
...
| `getFootsteps` | `event_id` | [{footstep_id: int, p100: [2d numpy array], grf: [1d numpy array]}] | Open the steps.npz file found in the swipe event directory and create the P100 and GRF numpy array |